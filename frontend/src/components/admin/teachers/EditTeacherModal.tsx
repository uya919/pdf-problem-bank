/**
 * EditTeacherModal - 강사 수정 모달
 *
 * Phase 9-3: Mock 데이터 기반 강사 수정
 * - 기존 데이터 pre-fill
 * - React Portal 사용 (z-index: 9999)
 */
import { useState, useEffect } from 'react';
import { createPortal } from 'react-dom';
import type { MockTeacher } from './CreateTeacherModal';
...
/** 과목 옵션 */
const SUBJECT_OPTIONS = ['수학', '국어', '영어', '과학', '사회', '물리', '역사', '기타'];

export function EditTeacherModal({ isOpen, teacher, onClose, onSave }: EditTeacherModalProps) {
  // 폼 상태
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updatedTeacher: MockTeacher = {
        ...teacher,
        name: name.trim(),
        subject,
        phone: phone.trim(),
      ...
      setError('강사 정보 수정 중 오류가 발생했습니다.');
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6">
            ...
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-bold text-grey-900">강사 정보 수정</h2>
              <button
                type="button"
                onClick={handleClose}
