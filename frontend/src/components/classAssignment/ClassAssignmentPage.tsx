/**
 * 과목별 반 배정 메인 페이지 (Phase 7-E)
 *
 * 과목 탭 + 학부 필터 + 학생 목록 + 반 컬럼
 * 드래그&드롭 및 단축키로 빠른 배정
 */
...
import { ClassColumn, ClassColumnSkeleton } from './ClassColumn';
    ...
    async (classId: string) => {
      ...
      try {
        ...
        const remainingUnassigned = unassignedStudents.filter(
          (s) => !selectedStudentIds.has(s.student_id)
        );
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              학생을 선택하고 반에 드래그하거나 단축키로 빠르게 배정하세요
            </p>
          </div>
          <ShortcutButton />
        </div>
            ...
            <div className="flex gap-4">
              {(['advanced', 'regular', 'regular2', 'basic'] as ClassLevel[]).map(
                (level) => {
                  ...
                  const classInfo = levelClasses[0]; // 첫 번째 반 사용
                  ...
                  return (
                    <ClassColumn
                      ...
                      onDrop={() => handleAssignToClass(classInfo.id)}
                    />
                  );
                }
              )}
